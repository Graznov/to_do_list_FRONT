...
export interface StyleState {
    ...
    pathToImg:string,
    noteWindRedactVisible:boolean,
    newNote_redactedNote:boolean,
    pencil:boolean
}
...
const initialState:StyleState = {
    ...
    noteWindRedactVisible:false,
    newNote_redactedNote:false,//для кнопки удаления записи, чтоб скпывалась при добапвлении новой записи
    pencil:false
}
...
const styleSlice = createSlice({
    ...
    reducers:{
        ...
        setNoteRedactWindVisible(state, action){
            state.noteWindRedactVisible=action.payload
        },
        setNewNote_redactedNote(state, action){
            state.newNote_redactedNote=action.payload
        },
